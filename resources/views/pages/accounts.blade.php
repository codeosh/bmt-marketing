                ...
                @foreach ($accounts as $account)
                    <tr data-id="{{ $account->id }}">
                        ...
                        <td class="px-3 py-2"><span
                                class="{{ $account->role === 'admin' ? 'badge bg-success' : 'badge bg-primary' }}">{{ $account->role }}</span>
                        </td>
                        <td class="px-3 py-2"><span
                                class="{{ $account->role === 'admin' ? 'badge bg-success' : 'badge bg-primary' }}">{{ $account->status }}</span>
                        </td>
                        <td class="px-3 py-2">
                            <div class="d-flex gap-2">
                                ...
                                <div class="deleteButton d-flex align-items-center justify-content-center">
                                    <button type="button" class="btn btn-danger"
                                        style="font-size:0.6rem; width:100px; height:25px; border-radius:3px"
                                        id="deleteAccBtn" data-id="{{ $account->id }}">
